...
public class QLearningController extends Controller {
	static final int PART = 3; /* PART = 2 >> No hovering; PART = 3 >> Hovering */
  ...
  static final double GAMMA_DISCOUNT_FACTOR = 0.85; //.95 /* Must be < 1, small values make it very greedy */
	static final double LEARNING_RATE_CONSTANT = 300; //10 /* See alpha(), lower values are good for quick results in large and deterministic state spaces */
  double explore_chance = 0.3; /* The exploration chance during the exploration phase */
  final static int REPEAT_ACTION_MAX = 40; /* Repeat selected action at most this many times trying reach a new state, without a max it could loop forever if the action cannot lead to a new state */
	...
	final static int NUM_ACTIONS = 5;

	/* Three-bit engine control definitions */
	final static int LEFT_ENGINE = 0x01;
	final static int RIGHT_ENGINE = 0x02;
  final static int MIDDLE_ENGINE = 0x04;

	/* Performs the chosen action */
	void performAction(int action) {

		/* Fire zeh rockets! */
		leftEngine.setBursting((action & LEFT_ENGINE) != 0);
    middleEngine.setBursting((action & MIDDLE_ENGINE) != 0);
    rightEngine.setBursting((action & RIGHT_ENGINE) != 0);
